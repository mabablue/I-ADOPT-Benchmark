...
`)).join(`
`):"-"}
]`}}class ht extends q{#e=[];addEntity(t){this.#e.push(t)}getEntities(){return this.#e.slice(0)}getClassLabel(){return"Constraint"}remove(){this.getVariable()._removeConstraint(this),this.#e.forEach(t=>t._removeConstraint(this))}toString(){return`[Constraint ${this._iri?`(${this._iri})`:"(_blank)"}`+(Object.values(this._label).length?`
  label:
`+Object.entries(this._label).map(([t,e])=>`    ${t}: ${e}`).join(`
`):"")+(Object.values(this._comment).length?`
  comment:
...
`+Object.entries(this._comment).map(([t,e])=>`    ${t}: ${e}`).join(`
`):"")+`
]`}}function yt(s){const t=new ct({iri:s["@id"],label:s.label,comment:s.comment}),e={};let i=D(s.property,vt,e);if(i&&t.setProperty(i),i=D(s.statisticalModifier,M,e),i&&t.setStatisticalModifier(i),i=D(s.ooi,M,e),i&&t.setObjectOfInterest(i),i=D(s.matrix,M,e),i&&t.setMatrix(i),s.context)for(const r of s.context)i=D(r,M,e),t.addContextObject(i);if(s.constraint)for(const r of s.constraint){let{constraint:c,entities:n}=Ct(r);n=n.map(m=>e[m]),t.addConstraint(c,...n)}return t}function D(s,t,e){if(!s)return;const i=new t({iri:s["@id"],label:s.label,comment:s.comment});e[i.getIri()]=i;for(const r of Q)if(r in s){const c=Array.isArray(s[r])?s[r].map(n=>D(n,t,e)):[D(s[r],t,e)];for(const n of c)i.addComponent(r,n),e[n.getIri()]=n}return i}function Ct(s){if(s)return{constraint:new ht({iri:s["@id"],label:s.label,comment:s.comment}),entities:s.constrains}}var xt=y('<a class="navBoxItem"><li> </li></a>'),Ot=y('<li class="navBoxSection svelte-10b7788"><label class="svelte-10b7788"><i> </i></label> <input type="checkbox" class="svelte-10b7788"/> <ul class="svelte-10b7788"></ul></li>'),St=rt(y(`<div class="navBox svelte-10b7788"><div class="navBoxHead">Filter</div> <div class="navBoxBody filterBody svelte-10b7788"><input type="text" id="filter" placeholder="Filter Variables ..."/> <script>
      (function(){
    ...
    <\/script></div></main>`,1));function Jt(s,t){nt(t,!0);const e=yt(t.data.variable),i=t.data.path.split("/").slice(0,-1).map(()=>"..").join("/");var r=zt(),c=Y(r),n=o(c);wt(n,{get selected(){return t.data.path},get variables(){return t.data.variableList}}),a(c);var m=h(c,2),x=o(m),j=o(x),V=o(j),L=o(V,!0);a(V);var B=h(V,2),F=o(B);{var P=f=>{var O=Pt();w(()=>R(O,"href",t.data.issue)),u(f,O)};I(F,f=>{t.data.issue&&f(P)})}var $=h(F,2),d=o($);a($),a(B);var l=h(B,2),_=o(l,!0);a(l),a(j);var b=h(j,2),g=o(b),S=h(o(g),2);{let f=X(()=>e.getProperty());H(S,()=>v(f))}var T=h(S,4);{let f=X(()=>e.getObjectOfInterest());H(T,()=>v(f))}var J=h(T,2);{var z=f=>{var O=$t(),N=h(Y(O),2);{let W=X(()=>e.getStatisticalModifier());H(N,()=>v(W))}u(f,O)};I(J,f=>{e.getStatisticalModifier()&&f(z)})}var A=h(J,2);{var G=f=>{var O=At(),N=h(Y(O),2);{let W=X(()=>e.getMatrix());H(N,()=>v(W))}u(f,O)};I(A,f=>{e.getMatrix()&&f(G)})}var C=h(A,2);{var p=f=>{var O=Tt(),N=h(Y(O),2);K(N,17,()=>e.getContextObjects(),Z,(W,mt)=>{H(W,()=>v(mt))}),u(f,O)};I(C,f=>{e.getContextObjects().length>0&&f(p)})}a(g),a(b),a(x);var k=h(x,4),U=o(k);dt(2),a(k),a(m),w((f,O,N)=>{E(L,f),R($,"href",`${i}/${t.data.path}`),R(d,"src",`${i}/rdf.svg`),E(_,O),R(U,"src",`https://sirkos.github.io/iadopt-vis/remote.html?jsonld=${N??""}`)},[()=>e.getLabel(),()=>e.getComment(),()=>encodeURIComponent(JSON.stringify(t.data.variable))]),u(s,r),at()}export{Jt as component};
